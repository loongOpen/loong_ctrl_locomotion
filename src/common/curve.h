...
public:
	swTrj6dClass(){}
	void setRP0(const mat3d &RR,const vec3d &pp){R0=RR;p0=pp;}//set起点
	void setRP0now(){R0=R;p0=p;}//以class内部当前位置为起点
	void setRP1(const mat3d &RR,const vec3d &pp,double T){R1=RR;p1=pp;this->T=T;}//一锤定音，T：达到目标时长，只会影响速度
	void setRP1(const mat3d &RR,const vec3d &pp,double T,double pgs);//有权重，适合实时set。pgs：轨迹更新过程progress，0~1
	void setH(const double h){this->h=h;}

	void update(double pgs,const double& yaw=0);//pgs：轨迹更新过程progress，0~1，pgs=t/T
	...
	void adjust(const mat3d &R,bool polish=1);
	...
	void getWV(vec6d &wv){wv=this->wv;}
